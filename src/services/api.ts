...
export const checkUserCredits = async (userId: string) => {
  try {
    const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/rest/v1/users?id=eq.${userId}&select=credits_remaining`, {
      method: 'GET',
      headers: {
    ...
    return { credits: userData.credits_remaining };
  } catch (error) {
...
export const recordCreditTransaction = async (
  ...
  amount: number,
  description: string,
  transactionType: 'usage' | 'purchase' | 'plan_allocation' | 'bonus' | 'refund'
) => {
  const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/rest/v1/credit_transactions`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'apikey': import.meta.env.VITE_SUPABASE_ANON_KEY,
